...
export class DetailComponent implements OnInit {
  ...
  private loadClaim(id: string): void {
    this.claim = {
      ...
      serviceLines: [
        { cptCode: '99213', units: 1, chargeAmount: 150, diagnosisPointers: ['A'], placeOfService: '11' }
      ],
      financials: {
        totalCharge: 150,
        balance: 150
  ...
  saveAndResubmit(): void {
    ...
    if (this.editMode && this.workingCopy) {
      ...
      this.claim.metadata.lastUpdatedAt = new Date();
      const diff = this.computeDiff(before, after);
      this.logAudit('Edited', `Changes saved prior to resubmission for claim ${this.claim.metadata.claimId}`, diff);
    }
    ...
    this.logAudit('StatusChanged', `Status changed ${previousStatus} -> Submitted for claim ${this.claim.metadata.claimId}`);
  ...
  withdrawClaim(): void {
    ...
    this.logAudit('StatusChanged', `Status changed ${previousStatus} -> Void for claim ${this.claim.metadata.claimId}`);
  }

  // ----- Audit utilities -----

  private logAudit(action: AuditEvent['action'], details: string, diff?: Record<string, unknown>): void {
    if (!this.claim) return;
    ...
    if (diff) {
      event.details += ' | Diff: ' + JSON.stringify(diff);
    }
  ...
  removeDiagnosis(index: number): void {
    if (!this.workingCopy) return;
    this.workingCopy.diagnoses = this.workingCopy.diagnoses.filter((_, i) => i !== index);
  }

  setPrimaryDiagnosis(index: number): void {
    if (!this.workingCopy) return;
    this.workingCopy.diagnoses = this.workingCopy.diagnoses.map((dx, i) => ({
      ...dx,
      primary: i === index
    }));
  }
  ...
  addServiceLine(): void {
    ...
    this.workingCopy.serviceLines = [
      ...(this.workingCopy.serviceLines || []),
      { cptCode: '', units: 1, chargeAmount: 0, diagnosisPointers: [], placeOfService: '' }
    ];
  ...
  removeServiceLine(index: number): void {
    if (!this.workingCopy) return;
    this.workingCopy.serviceLines = this.workingCopy.serviceLines.filter((_, i) => i !== index);
    this.recalcFinancials();
  }

  recalcFinancials(): void {
    if (!this.workingCopy) return;
    const total = (this.workingCopy.serviceLines || [])
      .reduce((sum, sl) => sum + (Number(sl.chargeAmount) || 0), 0);
